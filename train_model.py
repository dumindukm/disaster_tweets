...
from tensorflow.keras.layers import Flatten, Dense, Embedding, LSTM, Dropout, Bidirectional, Input, Concatenate
...
X_train_seq, X_test_seq, X_train_keyword, X_test_keyword, y_train, y_test = train_test_split(
    X_sequence, ds[["keyword_encoder"]], Y, test_size=0.33, random_state=42)
...
lstm_text = LSTM(15)(embedding_layer)

inp_keyword = Input(shape=(1,))
# lstm_keyword = LSTM(15)(inp_keyword)

layer = Concatenate()([lstm_text, inp_keyword])

layer = Dropout(.2)(layer)
layer = Flatten()(layer)
output = Dense(2, activation='softmax')(layer)

model = Model([inp_text_sequence, inp_keyword], [output])

modelCheckpoint = ModelCheckpoint('Checkpoints/tweet_disaster_model.h5',
                                  monitor='val_loss', verbose=0, save_best_only=True,  mode='auto', period=1)

model.compile(loss='sparse_categorical_crossentropy',
              optimizer='adam', metrics=['accuracy'])

# model.fit([X_train_seq, X_train_keyword], y_train, epochs=50, batch_size=50,
#           validation_split=.2, callbacks=[modelCheckpoint])

# model.save('tweet_disaster_model.h5')
model = load_model('Checkpoints/tweet_disaster_model.h5')
t_loss, t_accuracy = model.evaluate([X_test_seq, X_test_keyword], y_test)
